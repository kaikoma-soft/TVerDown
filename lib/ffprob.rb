...
module Mylib
  ...
  def ffprobe( input )
    ...
    r[ :fname ] = input
    r[ :comment ] = ""
    cmd = %W( ffprobe -pretty -hide_banner -show_streams #{input}  )
    IO.popen( cmd, :err=>[:child,:out] ) do |fp|
      fp.each_line do |line|
        ...
        key.each do |k|
          ...
          elsif line =~ /Stream #(\d:\d)\(jpn\): Audio/
            r[ :amap ] = $1 if r[ :amap ] == nil
